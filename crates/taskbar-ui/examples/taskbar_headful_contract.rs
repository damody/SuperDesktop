...
fn run() -> Result<(), String> {
    ...
    gpui::Application::with_platform(Rc::new(platform)).with_quit_mode(gpui::QuitMode::Explicit).run(move |cx:&mut App| {
        ...
        let opened=cx.open_window(options,move |window,cx|{
            ...
            cx.new(|_|TaskbarView{accessible_root_name:"SuperTaskbar".into(),layout:TaskbarLayout::calculate(2,primary_for_window.dpi_x,(primary_for_window.bounds.right-primary_for_window.bounds.left) as f32,&[],&[]),tasks:Vec::new(),status:status(),system_snapshot:None,system_flyout:None,notification_area:NotificationAreaModel::default(),overlays:Default::default(),show_labels:true,search_mode:TaskbarSearchMode::Hidden,show_task_view:true,alignment:TaskbarAlignment::Left,locked:true,callbacks:None,keyboard_focus:None,resize_subscription:None})
        });
