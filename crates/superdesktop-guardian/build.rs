...
fn main() {
    ...
    println!("cargo:rerun-if-changed=resources/superdesktop-guardian.rc");
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR must be set"));
    let icon = out_dir.join("superdesktop-guardian.ico");
    write_minimal_icon(&icon);
    let resource_script = out_dir.join("superdesktop-guardian.generated.rc");
    let icon_path = icon.display().to_string().replace('\\', "/");
    fs::write(
        &resource_script,
        format!(
            "{}\n201 ICON \"{}\"\n",
            include_str!("resources/superdesktop-guardian.rc"),
            icon_path
        ),
    )
    .expect("generated resource script must be writable");
    let output = out_dir.join("superdesktop-guardian.res");
    ...
    assert!(
        status.success(),
        "llvm-rc.exe failed to compile guardian resources"
    );
