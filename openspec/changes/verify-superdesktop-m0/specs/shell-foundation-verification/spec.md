...
驗證流程 SHALL 在任何視覺比較前記錄並驗證 Windows build、ExplorerPatcher 版本與 binary hash、影響工作列的設定匯出、參考影像及其 SHA-256；M0 主要 UI／互動基準 SHALL 為 Windows 11 build 26200.8875 與 ExplorerPatcher 26100.8457.70.3，Windows 10 22H2 SHALL 作為相容性目標。
...
### Requirement: Windows 10 22H2 相容性
驗證流程 SHALL 在 Windows 10 22H2 x64 驗證 preview 啟動、Shell mode 明確 opt-in、桌面與工作列操作、SuperExplorer 啟動、正常退出回復，以及 forced-crash guardian 回復 Explorer 與 work area。

#### Scenario: Windows 10 環境尚未提供
- **WHEN** 無法取得 Windows 10 22H2 x64 驗證環境
- **THEN** Windows 10 leaf SHALL 保持 blocked 且 M0 MUST NOT 宣告可發行

#### Scenario: Windows 10 強制崩潰回復
