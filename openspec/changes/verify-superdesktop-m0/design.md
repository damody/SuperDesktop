...
本 change 是 SuperDesktop M0 program 的最後一道發行驗證，不新增產品功能。所有 production child changes 必須先完成並封存，才能在凍結的參考環境、Windows 10 相容環境與螢幕拓撲矩陣上執行可重現的 gate。

主要 UI／互動參考環境為 Windows 11 build 26200.8875、ExplorerPatcher 26100.8457.70.3，以及 SHA-256 `48B5F990B9E155C5C2719D8F8B41D88ED4420A46C3B6018278511F9C349B387E` 的工作列截圖。Windows 10 22H2 x64 是相容性目標，不再是視覺像素基準。
...
- 驗證 Windows 10 22H2 的啟動、操作、正常回復與強制崩潰回復。
...
- 不因缺少 Windows 10 VM 或實體雙螢幕而降低門檻。
...
Windows 10 22H2 必須驗證 preview 啟動、Shell mode 明確 opt-in、核心互動、正常退出回復與 forced-crash guardian 回復。若環境未提供，相關 leaf 保持 `blocked`，M0 不得宣告完成。
...
- **Windows 10 外部環境不可用**：允許 leaf 保持 blocked，但不允許降低或略過 gate。
...
無。缺少外部 Windows 10 或實體雙螢幕環境是已知執行條件，不是規格待決事項。
