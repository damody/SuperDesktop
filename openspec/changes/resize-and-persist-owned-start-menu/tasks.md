...
- [x] 1.1.1 Add optional logical width and height to StartSettings defaults and public schema.
- [x] 1.1.2 Decode positive bounded integers and encode absent/value dimensions deterministically.
- [x] 1.1.3 Add legacy-missing, valid round-trip, malformed, and future-field settings tests.
...
- [x] 1.2.1 Implement shared default, minimum, maximum, and current-monitor size normalization.
- [x] 1.2.2 Apply normalized saved size to left/center and preview/shell Start positioning.
- [x] 1.2.3 Extend geometry tests across 96–192 DPI, rows 1–3, narrow work areas, and invalid sizes.
...
**輸入：** GPUI WindowOptions, vendored Windows popup style selection, window-bound observation, StartView lifecycle, and resize callback.
**產出：** Gated native popup resize style, 250ms generation fence, and backend/view tests.
**依賴：** 1.2.
...
- [x] 2.1.1 Enable native resizing in app and vendored Windows popup styles while retaining titlebar-free non-movable behavior.
- [x] 2.1.2 Add StartView bounds observation with unchanged suppression and 250ms generation-fenced debounce.
- [x] 2.1.3 Add popup style positive/negative, initial/no-op, continuous-drag, latest-value, and subscription-lifetime tests.
...
- [x] 2.2.1 Flush latest observed size on deactivation and before taskbar-toggle removal.
- [x] 2.2.2 Persist normalized dimensions atomically and update shared settings only after success.
- [x] 2.2.3 Add close-race, deactivation, unchanged, save-failure, and revision-reconciliation tests.
...
- [x] 3.1.1 Add a focused physical script for native resize, close/reopen, settings inspection, and process restart.
- [x] 3.1.2 Run two clean physical cycles against one candidate hash and inspect window containment/UIA.
- [x] 3.1.3 Scan reports/logs for write storms, stale settings, panic, borrow errors, and sensitive data.
...
- [x] 3.2.1 Run formatting, focused and locked/offline workspace tests, and Clippy warnings-as-errors.
- [x] 3.2.2 Run release build, installer without launch, and packaged app hash comparison.
- [x] 3.2.3 Write 18 unique evidence records, commit nested/parent revisions, and rerun strict/status checks.
