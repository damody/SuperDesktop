...
- [ ] 1.1.1 Add `C-W11-REFERENCE-001` with approval, scope, unchanged thresholds, and evidence invalidation rules.
- [ ] 1.1.2 Inventory every active non-archive Windows 10 requirement, task, script, schema, template, and evidence identifier.
- [ ] 1.1.3 Bind the canonical frozen-profile contract, settings, allowlist, ExplorerPatcher binaries, and reference image hashes.
- [ ] 1.1.4 Map each new requirement scenario to its implementation file, validation command, gate, and evidence subcheck.
...
- [ ] 1.2.1 Revise shell-foundation proposal, design, specs, tasks, and execution guidance without modifying archives.
- [ ] 1.2.2 Revise M0 verification proposal, design, specs, and pending platform task descriptions while preserving IDs.
- [ ] 1.2.3 Revise shell-completion proposal, design, spec, and pending task descriptions while preserving IDs.
- [ ] 1.2.4 Revise completion-program proposal, design, spec, tasks, and `PROGRAM.md` release language.
- [ ] 1.2.5 Add Windows 10 `not-claimed` classification and retain legacy tray and undocumented virtual-desktop limitations.
- [ ] 1.2.6 Produce an active-path scan proving no mandatory Windows 10 release statement remains.
...
- [ ] 2.1.1 Implement canonical contract loading and repository-contained referenced-file hash verification.
- [ ] 2.1.2 Implement live OS build/UBR, workstation/session, ExplorerPatcher version, binary, and settings observation.
- [ ] 2.1.3 Implement release-candidate ancestry plus committed, staged, and working-tree production drift rejection.
- [ ] 2.1.4 Emit a bounded owned observation containing profile fingerprint, candidate revision, and rehashable sources.
- [ ] 2.1.5 Add negative fixture tests for every independently failing admission field and obsolete Windows 10 input.
...
- [ ] 2.2.1 Rename the active lifecycle collector and output schema/path to reference-profile terminology.
- [ ] 2.2.2 Invoke shared admission before builds, Shell actions, operator acceptance, or evidence output.
- [ ] 2.2.3 Preserve preview zero-mutation, normal-exit, ten forced-crash, authenticated terminal, deadline, and final-baseline checks.
- [ ] 2.2.4 Replace Windows 10 host metadata with exact profile fingerprint and source hashes.
- [ ] 2.2.5 Add parser and wrong-profile fail-closed probes proving no output or Shell mutation.
...
- [ ] 2.3.1 Require shared profile admission before every `DryRun`, `Enable`, `AfterReboot`, and `Rollback` phase.
- [ ] 2.3.2 Replace the build-19045 mutation check with exact frozen-profile fingerprint admission.
- [ ] 2.3.3 Record profile sources and fingerprint in every installer phase artifact.
- [ ] 2.3.4 Preserve apply, explicit-opt-in, exact fingerprint, rollback metadata, and exact-absence restoration controls.
- [ ] 2.3.5 Run packaged installer dry-run and verify Registry value presence/value and rollback-record absence are unchanged.
- [ ] 2.3.6 Add negative probes for phase profile, candidate, binary, operator, rollback path, and fingerprint mismatch.
...
- [ ] 2.4.1 Rename the completion finalizer, parameters, schemas, kind, and output to `reference-profile-lifecycle-installer`.
- [ ] 2.4.2 Revise the roll-up JSON schema and collector external-source admission enum and validation.
- [ ] 2.4.3 Revise operator README, confirmation templates, readiness record, and release-candidate purpose.
- [ ] 2.4.4 Add fixtures proving old Windows 10 kinds and partial/mismatched phase sets are rejected.
- [ ] 2.4.5 Validate normalized gate derivation keeps physical mixed-DPI and independent review external pending.
...
- [ ] 3.1.1 Replace Windows 10 artifact lookup, schema checks, task derivation, and blocker text in local M0 capture.
- [ ] 3.1.2 Replace Windows 10 status and evidence paths in foundation roll-up capture.
- [ ] 3.1.3 Preserve five-DPI, physical mixed-DPI, independent-review, archive, and final-disposition dependencies.
- [ ] 3.1.4 Regenerate local blocker/evidence artifacts without marking unexecuted external tasks passed.
- [ ] 3.1.5 Validate every migrated M0/foundation task ID has unique evidence or a unique shared subcheck.
...
- [ ] 3.2.1 Recompute shell-completion local roll-up with the revised schema and external-source names.
- [ ] 3.2.2 Recompute completion-program lineage, child counts, commits, limitations, and blockers.
- [ ] 3.2.3 Verify old external kinds, missing children, duplicate gates, stale hashes, and contradictory decisions fail.
- [ ] 3.2.4 Record the exact remaining blocker set without inventing physical or independent evidence.
...
- [ ] 4.1.1 Parse every revised PowerShell script with Windows PowerShell 5.1-compatible syntax.
- [ ] 4.1.2 Run all reference-profile and obsolete-artifact negative probes.
- [ ] 4.1.3 Run `cargo fmt`, workspace tests, and workspace clippy with warnings denied.
- [ ] 4.1.4 Run detailed task validation for this change and affected detailed active changes.
- [ ] 4.1.5 Run `openspec validate --all --strict` and active-path Windows 10 requirement scan.
- [ ] 4.1.6 Verify no Registry, rollback metadata, reboot, archive, or external-service mutation occurred.
