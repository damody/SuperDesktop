...
Each admitted artifact changes only its mapped gates to `passed`; missing artifacts remain `external_pending`, so incremental recomputation never weakens the final gate. Once any external artifact exists, the collector rejects unknown JSON artifacts, wrong hosts, revisions other than the frozen candidate, tracked production or verification-tool drift after the candidate, malformed or internally partial evidence, source/artifact hash drift, unresolved P0/P1 findings, incomplete interaction matrices, recovery slower than ten seconds, and inexact installer rollback. Only change-local `evidence/` and `tasks.md` updates may descend from the candidate without invalidating the reviewed revision. Example confirmations are templates only and are rejected until every placeholder is replaced by attributable data.
