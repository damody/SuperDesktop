...
- [x] 1.1.1 實作 monitor identity 到 GPUI desktop window 的 lifecycle mapping。
- [x] 1.1.2 實作 DPI-aware bounds、z-order 與 non-activating desktop semantics。
- [x] 1.1.3 實作 monitor add/remove、primary change 與 DPI change reconciliation。
- [x] 1.1.4 建立單螢幕與虛擬多螢幕 headful lifecycle tests。
- [x] 1.1.5 保存 window identity、geometry 與 teardown resource evidence。
...
- [x] 1.2.1 實作 solid color 與圖片 decode error fallback。
- [x] 1.2.2 實作 Fill、Fit、Stretch 與 Center geometry。
- [x] 1.2.3 實作 Tile 與跨 monitor Span geometry。
- [x] 1.2.4 實作 bounded decoded-image cache 與 invalidation。
- [x] 1.2.5 產生各模式/DPI visual fixtures 與 geometry assertions。
...
- [x] 2.1.1 實作 User Desktop 與 Public Desktop known-folder enumeration。
- [x] 2.1.2 將 PIDL/COM 結果轉成 owned stable Shell identity 與 capabilities。
- [x] 2.1.3 實作 identity-based merge、display-name 與 icon descriptor mapping。
- [x] 2.1.4 加入同名、Unicode、hidden 與 system item 獨立 fixtures。
- [x] 2.1.5 驗證 UI crate 不持有 COM/PIDL 並保存 architecture evidence。
...
- [x] 2.2.1 實作 Win10 參考尺寸的 DPI-aware icon/label grid。
- [x] 2.2.2 實作單選、Ctrl toggle、Shift range 與 rubber-band selection。
- [x] 2.2.3 實作 drag reposition、collision resolution 與 logical-coordinate persistence。
- [x] 2.2.4 實作 monitor 缺失、DPI change 與 work-area change 的 deterministic clamp。
- [x] 2.2.5 建立重啟、DPI 轉換與 monitor remap tests。
- [x] 2.2.6 保存各 DPI geometry、selection 與 persisted settings evidence。
...
- [x] 2.3.1 實作「SuperExplorer」固定入口的 stable identity、icon、label 與 layout。
- [x] 2.3.2 綁定 pointer、Enter 與 UIA invoke 到同一 default bridge command。
- [x] 2.3.3 以 fake bridge 驗證 launched、validation/spawn failure、cancel 與 timeout states。
- [x] 2.3.4 執行多 surface/DPI existence、focus、accessible name/role/action headful tests。
...
- [x] 3.1.1 實作 Enter、方向鍵與 selection keyboard commands。
- [x] 3.1.2 實作 pointer double-click、rubber-band 與 position-only drag commands。
- [x] 3.1.3 預留 rename/context/delete/refresh 的 typed unavailable states 而不執行 effect。
- [x] 3.1.4 驗證 F2、Delete、F5 與 context-menu gesture 不暴露未實作控制或造成檔案 mutation。
- [x] 3.1.5 驗證 drag 只改 icon position，不啟動 file transfer。
- [x] 3.1.6 執行 keyboard-only、pointer 與 UIA M0 action headful tests。
...
- [x] 3.2.1 實作 folder capability 到 typed bridge launch request 的 mapping。
- [x] 3.2.2 綁定 Enter 與 double-click 到同一個 folder launch command。
- [x] 3.2.3 以 fake bridge 驗證 launched 與 validation/spawn failure terminal UI。
- [x] 3.2.4 以 fake bridge 驗證 cancel/timeout 與 late-terminal suppression。
...
- [x] 3.3.1 實作 owned Shell identity 到 Windows association request adapter。
- [x] 3.3.2 實作 success、validation-failed、launch-failed、cancelled、timed-out terminal mapping。
- [x] 3.3.3 加入 cancel-vs-success 與 timeout-vs-late-callback tests。
- [x] 3.3.4 實作 GPUI repair prompt 且不 fallback 到 Windows Explorer。
- [x] 3.3.5 執行真實 fixture file association integration test。
...
- [x] 3.4.1 對 watcher callback 套用 Wave 2 凍結的 no-unwind wrapper 並轉成 owned event。
- [x] 3.4.2 實作 overflow 到 full namespace refresh 的單次觸發。
- [x] 3.4.3 加入外部 rename storm 與 stale completion suppression tests。
- [x] 3.4.4 驗證 refresh 後依 stable identity 恢復 selection/position。
- [x] 3.4.5 保存 raw watcher trace、queue depth 與 final snapshot diff。
...
- [x] 4.1.1 執行單螢幕 100% 至 200% DPI visual geometry matrix。
- [x] 4.1.2 執行虛擬多螢幕 topology 與 desktop interaction matrix。
- [x] 4.1.3 執行 accessibility identity、focus 與 action scan。
- [x] 4.1.4 執行 desktop resource teardown 與 cache-bound verification。
- [x] 4.1.5 索引全部 artifact hashes 並發布 `G-DESKTOP` disposition。
- [x] 4.1.6 產生 desktop public/effect schema、binary、evidence-index 與 handoff SHA-256 manifest。
