...
- Replacement input set 以每個 predecessor manifest 的 path 聯集為不可縮減下限；依賴或 vendor 更新後必須同步重生完整 locked metadata、license inventory與direct dependency provenance，並使 source-boundary audit通過。Aggregate handoff contract必須有明確 inputs manifest、演算法及可由工具重算的檔案 hash。
...
- Replacement 與 adjustment graph 都必須檢查間接循環；fixture 必須命中 dangling、cycle、nonmandatory、coverage-drift、unpassed 等專屬語意 diagnostic，不能以通用 mandatory-missing failure 提前遮蔽。
