...
SuperDesktop SHALL activate the Windows-registered built-in image-snipping overlay through `IApplicationActivationManager::ActivateForProtocol` using only the fixed built-in AUMID `Microsoft.ScreenSketch_8wekyb3d8bbwe!App` and observed native-hotkey URI `ms-screenclip:///?source=HotKey`; it SHALL NOT use ShellExecute, resolve or launch Explorer, discover an executable path, register a capture callback, or launch a third-party capture program.
