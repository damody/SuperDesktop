...
目前 `D:\SuperDesktop` 只有可供行為研究的舊 C++/Win32 PExplorer，尚無可維護、可驗證且以 Rust/GPUI 實作的 Windows 桌面環境。現在需要先建立 SuperDesktop 的 M0 基礎，使桌面、雙列工作列、SuperExplorer 整合及 Shell 異常復原形成可實際套用與驗收的產品骨架，再由後續 change 擴充完整 Windows Shell 功能。
...
- 目前開發機的 Windows 11＋ExplorerPatcher UI 是 M0 與 release reference profile；真實 mixed-DPI 雙螢幕另作 release-candidate confirmation。Windows 10 compatibility 不在本 release claim 內。虛擬顯示器可先完成自動化 topology gate，外部環境缺失不阻擋 production changes，但 final confirmation 保持 blocked。
