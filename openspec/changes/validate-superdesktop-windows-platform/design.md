...
- Windows executable 必須啟用 pinned `gpui` 的 `windows-manifest` feature，以嵌入同 revision 已審核的 Common Controls v6 manifest；未啟用時 loader 會以 `STATUS_ENTRYPOINT_NOT_FOUND` 找不到 `comctl32!TaskDialogIndirect`，不得將該失敗誤判為 GPU/DXGI capability failure。
