...
if((RequireField $trace 'schema') -ne 'monitor-dpi-start-trace/v2' -or (RequireField $trace 'mode') -ne 'read-only-preview'){throw 'TRACE_SCHEMA_OR_MODE'}
RequireBool $trace 'explorer_mutations' $false;RequireBool $trace 'shell_takeover' $false;RequireBool $trace 'start_invocation_attempted' $false
if((RequireField $trace 'typed_disposition') -ne 'stop'){throw 'TRACE_DISPOSITION_NOT_STOP'}
...
$start=RequireField $trace 'start_host';if((RequireField $start 'status') -ne 'unavailable'){throw 'TRACE_START_STATUS'};$reason=RequireField $start 'reason';if($reason -notin @('taskbar-host-not-found','untrusted-start-host-invocation-refused')){throw 'TRACE_START_UNAVAILABLE_NOT_TRUTHFUL'};RequireBool $start 'invocation_attempted' $false;if((RequireField $start 'disposition') -ne 'stop'){throw 'TRACE_START_DISPOSITION'}
$observation=RequireField $start 'host_observation';if((RequireField $observation 'path') -ne $null){throw 'TRACE_START_PATH_UNTRUSTED'};if($observation.taskbar_class -ne $start.observed_taskbar_class){throw 'TRACE_START_OBSERVATION_MISMATCH'}
...
$paths=[ordered]@{current_substrate_manifest_sha256=Join-Path $WorkspaceRoot 'openspec/changes/validate-superdesktop-windows-platform/evidence/artifacts/1.1/current-substrate-inputs-successor-1.2-manifest-v3.sha256';pre_mutation_admission_trace_sha256=Join-Path $ArtifactDirectory 'pre-mutation-admission-trace.json';binary_sha256=Join-Path $ArtifactDirectory 'bin/monitor_dpi_start_capability.exe';runner_source_sha256=Join-Path $WorkspaceRoot 'crates/platform-win/examples/monitor_dpi_start_capability.rs';adapter_source_sha256=Join-Path $WorkspaceRoot 'crates/platform-win/src/common/monitor_dpi_start.rs'}
...
Write-Output 'Monitor/DPI/Start trace semantic validation passed; taskbar capability remains typed unavailable/stop.'
