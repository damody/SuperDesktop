[CmdletBinding()]
param([string]$WorkspaceRoot,[string]$OutputPath,[string]$CurrentSubstratePath,[string]$ProbeBinaryPath,[string]$AdmissionTracePath)
...
$currentSubstrate = if($CurrentSubstratePath){$CurrentSubstratePath}else{Join-Path $artifacts 'current-substrate-inputs-successor-1.2.sha256'}
$probeBinary = if($ProbeBinaryPath){$ProbeBinaryPath}else{Join-Path $artifacts 'bin/capability_profile-successor-1.2.exe'}
$admissionTrace = if($AdmissionTracePath){$AdmissionTracePath}else{Join-Path $artifacts 'admission-zero-mutation-trace-successor-1.2.json'}
...
$output = [ordered]@{
  ...
  build = [ordered]@{ gpui_revision='8945e2981b9fd00ca887e042d8adb9acc241b168'; rust_toolchain_path='rust-toolchain.toml'; rust_toolchain_sha256=&$sha (Join-Path $WorkspaceRoot 'rust-toolchain.toml'); cargo_lock_path='Cargo.lock'; cargo_lock_sha256=&$sha (Join-Path $WorkspaceRoot 'Cargo.lock'); probe_source_path='crates/platform-win/examples/capability_profile.rs'; probe_source_sha256=&$sha (Join-Path $WorkspaceRoot 'crates/platform-win/examples/capability_profile.rs'); probe_binary_path='evidence/artifacts/1.1/bin/capability_profile-successor-1.2.exe'; probe_binary_sha256=&$sha $probeBinary }
  current_substrate_contract = [ordered]@{ path='evidence/artifacts/1.1/current-substrate-inputs-successor-1.2.sha256'; sha256=&$sha $currentSubstrate; verifier_path='scripts/verify-current-substrate-contract.ps1'; verifier_sha256=&$sha (Join-Path $PSScriptRoot 'verify-current-substrate-contract.ps1'); predecessor_archive_contract_sha256=[string]$programHandoff.child_contract_sha256 }
  ...
  admission_zero_mutation_trace = [ordered]@{ path='evidence/artifacts/1.1/admission-zero-mutation-trace-successor-1.2.json'; sha256=&$sha $admissionTrace }
}
$destination=if($OutputPath){$OutputPath}else{Join-Path $artifacts 'frozen-profile-contract-successor-1.2.json'}
