...
$out=if($OutputPath){$OutputPath}else{Join-Path $WorkspaceRoot 'openspec/changes/validate-superdesktop-windows-platform/evidence/artifacts/1.1/current-substrate-inputs-successor-1.2-manifest-v3.sha256'}
$provenanceOut=if($ProvenanceOutputPath){$ProvenanceOutputPath}else{Join-Path $WorkspaceRoot 'openspec/changes/validate-superdesktop-windows-platform/evidence/artifacts/1.1/current-substrate-v3-vendor-provenance.json'}
...
function Get-ExpectedPaths {
  @(
    ...
    'vendor/gpui/.cargo-checksum.json','vendor/gpui/Cargo.toml','vendor/gpui/LICENSE-APACHE','vendor/gpui/build.rs','vendor/gpui/resources/windows/gpui.manifest.xml','vendor/gpui/resources/windows/gpui.rc',
    ...
    'openspec/changes/validate-superdesktop-windows-platform/evidence/artifacts/1.1/current-substrate-v3-vendor-provenance.json'
  )
}
...
[ordered]@{schema_version='1.0.0';contract='B-W2-1.2-007-current-substrate-v3-vendor-license-provenance';cargo_lock_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $WorkspaceRoot 'Cargo.lock')).Hash;gpui=[ordered]@{vendor_path='vendor/gpui';license='Apache-2.0';checksum_path='vendor/gpui/.cargo-checksum.json';build_script_path='vendor/gpui/build.rs';resources=@('vendor/gpui/resources/windows/gpui.manifest.xml','vendor/gpui/resources/windows/gpui.rc')};packages=$packages} | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $provenanceOut -Encoding utf8
...
Write-Output "Current substrate v3 provenance written: $provenanceOut"
Write-Output "Current substrate v3 exact-set manifest written: $out"
