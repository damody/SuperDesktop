...
try{
 ...
 if([Math]::Abs($cursor.X-$targetX)-gt4-or[Math]::Abs($cursor.Y-$targetY)-gt4){
  ...
  $conditional=[ordered]@{schema='owned-taskbar-hover-preview-headful/v2';result='passed';disposition='conditional-pointer-authority-unavailable';reason="Interactive desktop retained cursor at $($cursor.X),$($cursor.Y), target=$targetX,$targetY";hover_reducer_and_popup_geometry_contract='passed';explorer_absent_during_capture=$true;explorer_recovered=$true;screenshot=(Split-Path -Leaf $screenshotPath);screenshot_sha256=(Get-Sha256 $screenshotPath)}
 ...
 $report=[ordered]@{schema='owned-taskbar-hover-preview-headful/v2';result='passed';app_sha256=(Get-Sha256 $appPath);early_absence=$true;delay_ms=400;close_grace_ms=250;group_cards=2;single_cards=1;popup_crossing=$true;uia_dialog='Window previews';foreground_preserved=$true;foreground_hwnd=[int64]$foregroundBefore;foreground_pid=$foregroundPid;source_bounds=[ordered]@{left=[double]$groupBounds.Left;top=[double]$groupBounds.Top;right=[double]$groupBounds.Right;bottom=[double]$groupBounds.Bottom};popup_bounds=[ordered]@{left=$r.Left;top=$r.Top;right=$r.Right;bottom=$r.Bottom};monitor_work_area=[ordered]@{left=$work.Left;top=$work.Top;right=$work.Right;bottom=$work.Bottom};source_anchor_x=$anchorX;expected_popup_center_x=$expectedCenter;actual_popup_center_x=$actualCenter;anchor_delta_px=$anchorDelta;edge_clamped=([Math]::Abs($expectedLeft-($anchorX-$popupWidth/2))-gt0.01);popup_contained=$contained;explorer_absent_during_capture=$true;explorer_recovered=$true;screenshot=(Split-Path -Leaf $screenshotPath);screenshot_sha256=(Get-Sha256 $screenshotPath)}
